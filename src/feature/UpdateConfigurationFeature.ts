...
export class UpdateConfigurationFeature implements IFeature {
  ...
  constructor(
    ...
    this.logger = logger;
    context.subscriptions.push(vscode.commands.registerCommand(PuppetCommandStrings.PuppetUpdateConfigurationCommandId,
      (updateSettingsHash) => { this.updateSettingsAsync(updateSettingsHash); }
    ));
  }
