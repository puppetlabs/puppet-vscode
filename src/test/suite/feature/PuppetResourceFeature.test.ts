...
suite('PuppetResourceFeature Test Suite', () => {
  ...
  test('run should show information message when language server is not ready', () => {
  ...
  test('run should not proceed when no resource name is provided', () => {
    sandbox.stub(vscode.window, 'showInputBox').resolves(undefined);
    puppetResourceFeature.run();
    assert.isTrue(editCurrentDocumentStub.notCalled);
  });

  test('run should call editCurrentDocument when resourceResult.data is not empty', async () => {
  ...
  test('run should log error when resourceResult.error is not empty', () => {
    sandbox.stub(vscode.window, 'showInputBox').resolves('test-resource');
